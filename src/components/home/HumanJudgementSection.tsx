...
export function HumanJudgementSection(): React.ReactElement {
  return (
    <Panel tone="ink" index="04" kicker="Human judgement">
      <Reveal className="max-w-4xl">
        ...
        <p className="mt-6 max-w-3xl text-base leading-relaxed text-bone-dim sm:text-lg">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        <Reveal className="rounded-2xl border border-white/10 bg-ink-raised p-7">
          <p className="kicker text-iris">Humans keep</p>
          <ul className="mt-5 space-y-3">
            {HUMANS.map((h) => (
              <li key={h} className="flex items-start gap-3 border-t border-white/10 pt-3 font-serif text-xl text-bone-text">
                {h}
              </li>
            ))}
          </ul>
        </Reveal>
        <Reveal delay={0.08} className="rounded-2xl border border-iris/50 bg-[rgba(169,156,255,0.08)] p-7 shadow-[var(--iris-emph)]">
          <p className="kicker text-iris">Looped handles</p>
          <ul className="mt-5 space-y-3">
            {LOOPED.map((l) => (
              <li key={l} className="flex items-start gap-3 border-t border-white/10 pt-3 font-serif text-xl text-bone-text">
                {l}
              </li>
            ))}
          </ul>
