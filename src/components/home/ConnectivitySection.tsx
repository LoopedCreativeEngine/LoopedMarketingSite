...
export function ConnectivitySection(): React.ReactElement {
  return (
    <Panel tone="night" kicker="Connectivity">
      ...
      <RevealStagger className="mt-12 grid gap-4 md:grid-cols-3">
        {BANDS.map((b, i) => (
          <RevealItem
            key={b.label}
            className={cn(
              "flex flex-col rounded-[20px] bg-night-raised p-6 shadow-[var(--lift-ink)] sm:p-7",
              i === 2 ? "border-2 border-pink/50" : "border border-white/10",
            )}
          >
            <div className="flex items-center justify-between">
              <p className="kicker text-lavender">{b.label}</p>
              <div className="flex gap-1" aria-hidden>
                {[0, 1, 2].map((seg) => (
                  <span
                    key={seg}
                    className={cn("h-1.5 w-5 rounded-full", seg < b.fill ? "bg-grad" : "bg-white/15")}
                  />
                ))}
              </div>
            </div>
            <p className="mt-4 text-xs font-medium uppercase tracking-[0.04em] text-mist-dim">{b.inputs}</p>
            <p className="mt-4 flex-1 font-serif text-lg leading-snug text-snow">{b.output}</p>
          </RevealItem>
        ))}
      </RevealStagger>
