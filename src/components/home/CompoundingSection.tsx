...
export function CompoundingSection(): React.ReactElement {
  return (
    <Panel tone="bone" index="07" kicker="Portfolio learning">
      ...
      <Reveal className="mt-14 overflow-hidden rounded-2xl border border-[rgba(23,19,31,0.12)] bg-paper shadow-[var(--lift-light)]">
        <div className="grid divide-y divide-[rgba(23,19,31,0.10)] md:grid-cols-4 md:divide-x md:divide-y-0">
          {EDITIONS.map((e, i) => (
            <div key={e.label} className={`relative p-7 sm:p-8 ${i === EDITIONS.length - 1 ? "bg-[rgba(67,56,202,0.05)]" : ""}`}>
              {i === EDITIONS.length - 1 ? <span className="absolute right-6 top-7 h-2 w-2 rounded-full bg-violet" aria-hidden /> : null}
              <div className="flex h-24 flex-col-reverse gap-1.5" aria-hidden>
                {Array.from({ length: i + 1 }).map((_, layer) => (
                  <div key={layer} className={`h-4 w-full rounded-sm ${layer === i ? "bg-violet" : "bg-violet/15"}`} />
                ))}
              </div>
              <p className="mt-5 font-serif text-xl text-ink-text">{e.label}</p>
              <p className="mt-2 text-sm leading-relaxed text-graphite">{e.inherits}</p>
            </div>
          ))}
        </div>
      </Reveal>
