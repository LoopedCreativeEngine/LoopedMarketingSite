    ...
    <div className="bg-bone pb-24 pt-28 sm:pt-32">
      <article className="mx-auto max-w-2xl px-5 sm:px-6 lg:px-8">
        <Reveal>
          <Link
            href="/newsroom"
            className="inline-flex items-center gap-1.5 kicker text-violet transition-colors hover:text-ink-text"
          >
            <span aria-hidden>←</span>
            Newsroom
          </Link>

          <div className="mt-8 flex flex-wrap items-center gap-3 kicker text-muted-ink">
          ...
          <h1 className="mt-5 text-balance font-serif text-4xl leading-tight tracking-tight text-ink-text sm:text-5xl">
            {article.title}
          </h1>
          <p className="mt-6 font-serif text-xl italic leading-relaxed text-violet sm:text-2xl">{article.standfirst}</p>
          <p className="mt-6 border-t border-[rgba(23,19,31,0.12)] pt-5 text-sm text-muted-ink">{article.byline}</p>
        </Reveal>

        <Reveal className="mt-10 space-y-6">
          {article.paragraphs.map((paragraph, index) => (
            <p key={index} className="text-base leading-[1.85] text-graphite sm:text-lg">
              {paragraph}
            </p>
          ))}
        </Reveal>

        <Reveal className="mt-14 border-t border-[rgba(23,19,31,0.12)] pt-10 text-center">
